                ...
                <div class="col-xs-6">
                    <div class="form-group">
                        ...
                        <div class="input-group mg-b-md">
                            ...
                            <input readonly type="text" name="mog_number" value="<?php echo empty($mog) ? set_value('mog_number') : $mog->mog_number; ?>" class="form-control" placeholder="BPM Number">
                        </div>
                    </div>
                </div>
            ...
            <div class="col-md-12">
                <?php if (!empty($mog)) { ?>
                    <div class="<?php echo!empty($mog) ? 'col-xs-6' : null ?>">
                        <a href="<?php echo base_url()?>warehouse/transaction_exit" style="cursor: pointer" class="cancelMt btn btn-danger col-xs-12"><i class="fa fa-times mg-r-sm"></i> Cancel </a>
                    </div>
                <?php } ?>
...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        $.ajax({
            url: "<?php echo base_url('warehouse/project/getdata'); ?>/" + $("select[name=project] option:selected").val() + '/2',
            dataType: "JSON",
        ...
        $(".cancelMt").click(function() {
            ...
            <?php if ($sess['position_id'] == 1 && !empty($mog)) { ?>
            ...
            function adds_row() {
                ...
                var baris = '<tr class="row_out row_tam">'
                    ...
                    $('#material_select_' + counter).load("<?php echo base_url() ?>warehouse/transaction_exit/material/" + $("select[name=project] option:selected").val() +"/"+ counter);
                }
...
//                        bootbox.alert("<i class='fa fa-warning mg-r-sm'></i>Nomor Surat Jalan tidak boleh kosong");
                        ...
                        $.ajax({
                            ...
                            success: function(json) {
                                if (json.status == 0) {
                                    ...
                                    <?php if ($sess['position_id'] == 1) { ?>
                    window.open("<?php echo base_url() ?>warehouse/formcontrollertemplate/bpm/" + json.id);
                    <?php } ?>
                                    $(".load_main_data").load("<?php echo base_url() ?>warehouse/transaction_exit/form");
                                }
                            }
