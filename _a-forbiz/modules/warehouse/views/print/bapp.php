...
$fpdf->SetXY(5, 2.43);
$fpdf->tabelbbc(array("SURAT PENERIMAAN BARANG ALAT"), 1);

$fpdf->SetWidths(array(1,2,1.4,1.6,1.3,2));
$date=$tdet->equipt_transaction_date;
$date= date('Y');
$month=  date('m');
$month= number_format_romawi($month);
$fpdf->SetXY(6, 3.2);

$fpdf->tabelbb(array("NO :","  $tdet->equipt_transaction_number","/    WG","/    UTR","/     ".$month,"/    ".$date), 0.5);
...
$fpdf->SetWidths(array(3,9));

$fpdf->SetXY(20,3.45);
if (!empty($detail->apps_logo)) {
    $fpdf->Image(base_url() . 'assets/img/apps/' . $detail->apps_logo, 0.5, 0.6, 4.6, 1);
//    $fpdf->Image(base_url().'assets/img/wika_gedungSMALL.png',0.2,0.5,-120,-150);
}
...
$fpdf->SetXY(0.5,3.82);
$fpdf->tabelbb(array("Diterima dari",": $tdet->actor_name"), 0.5);
$fpdf->SetXY(12,3.82);
...
$fpdf->SetX(0.5,4.25);
$fpdf->tabelbb(array("No Kendaraan",": ". strtoupper($tdet->equipt_transaction_car)), 0.5);
$fpdf->SetXY(12,4.7);
$fpdf->tabelbb(array("KTP/SIM",": $tdet->equipt_transaction_driver_identity"), 0.5);

$fpdf->Ln(0.3);
$fpdf->SetWidths(array(1,5,2,6.5,4.6));
$fpdf->SetX(0.5);
...
$fpdf->SetWidths(array(1,5,2,3,1.5,2,4.6));
foreach ($transaction as $i => $row) {
    $fpdf->SetFont("Times", "", 8);
    $fpdf->SetX(0.5);
$fpdf->tabel2(array(++$i,"$row->equipment_name","$row->equipment_type", !empty($row->equipt_transaction_dt_condition)?ucwords($row->equipt_transaction_dt_condition):"-","$row->equipt_transaction_dt_volume","$row->equipment_unit_name",""), 0.5  );
...
$fpdf->SetFont("Times", "", 8);
$fpdf->SetX(0.5);
$fpdf->tabelbb(array("CATATAN : APABILA BARANG YANG DITERIMA TIDAK SESUAI DENGAN PENGIRIMAN DARI SUPLIER/SUBKONT MAKA						
	DIBERIKAN TULISAN PADA KOLOM KETERANGAN"), 0.5  );
