    ...
    function toWords($amount, $major = MAJOR, $minor = MINOR) {
        ...
        if ($this->pounds == 0)
            ...
            for ($this->magind = 0; $this->magind < count($groups); $this->magind++) {
                if (($this->magind == 1) && (strpos($this->words, 'seratus') === false) && ($groups[0] != '000'))
...
//$fpdf->SetX(16);
//$fpdf->tabelbb(array(" Telp $detail->apps_phone, Fax $detail->apps_phone"), 0.5);
//$fpdf->SetWidths(array(19));
//$fpdf->Ln(0.5);
//$fpdf->SetFont("Arial", "", 12);
//$fpdf->tabelbbc(array("TANDA TERIMA BERKAS TAGIHAN"), 1);
//$fpdf->SetFont("Arial", "", 10);
//$fpdf->tabelbb(array("Telah diterima berkas tagihan dari : $salary->actor_name"), 0.5);
//$fpdf->tabelbb(array("Untuk Proyek : $salary->project_name"), 0.5);
//$fpdf->tabelbb(array("Yang terdiri dari : "), 0.5);
//$fpdf->tabel(array(""), 0.05);
//$y=$fpdf->GetY();
//$fpdf->SetWidths(array(10));
//$fpdf->SetXY(10, $y);
//$fpdf->tabelbb(array("Catatan :"), 0.5);
//$fpdf->SetX(10);
...
//$fpdf->SetX(9.8);
//$fpdf->tabel(array(""), 1);
//$fpdf->SetWidths(array(0.5,4,7));
//$fpdf->SetY($y);
//$fpdf->tabelbb(array("1.","Kwitansi Nomor",": Rp.$salary->salary_total"), 0.5);
...
//$fpdf->Ln(0.5);
//$fpdf->tabel(array(""), 0.05);
//$fpdf->SetWidths(array(5,9.5,5));
//$fpdf->tabelbbc(array("","","$salary->project_region,".  indo_date($salary->salary_date_kwt)), 0.5);
//$fpdf->SetWidths(array(5,9.5,5));
...
//ppn
    for ($f = 1; $f <= 2; $f++) {
        ...
        $fpdf->tabel(array("$salary->project_name"), 0.5);
...
//$fpdf->Ln(0.5);   
        ...
        $fpdf->SetX(16.4);
        $fpdf->tabelrr(array("Rp " . rupiah($ppn->salary_total_final)), 0.75);
//nomor spk
        $fpdf->SetXY(8.1, 3.43);
        $fpdf->SetWidths(array(15));
        $fpdf->tabelbb(array(""), 0.5);
        $fpdf->SetWidths(array(15));
        $amount = $ppn->salary_total_final;
        ...
        $fpdf->SetXY(1, 6.7);
        $fpdf->tabelbbcustom(array(!empty($ppn->salary_note) ? $ppn->salary_note : "-", "", "Rp " . rupiah($ppn->salary_total_final)), 0.5);
        ...
        if (empty($salary->salary_date)) {
            ...
            $salary_date = indo_date($salary_date);
        }
        $region = !empty($salary->project_region) ? ucwords(strtolower($salary->project_region)) : "______________";
        $fpdf->tabelbbr(array("$region, $salary_date"), 0.4);
...
//$fpdf->Ln(0.5);   
        ...
        $fpdf->SetX(16.45);
        $fpdf->tabelrr(array("Rp " . rupiah($ppn->salary_tax_nominal)), 0.75);
//nomor spk
        $fpdf->SetXY(8.1, 3.43);
        $fpdf->SetWidths(array(15));
        $fpdf->tabelbb(array(""), 0.5);
        $fpdf->SetWidths(array(15));
        $amount = $ppn->salary_tax_nominal;
        $obj = new toWords($amount);
        $nilai_terbilang = $obj->words;
        $fpdf->SetXY(1, 11.08);
        $fpdf->Image(base_url() . 'assets/img/bgBilangan.png', 3, 11, 12, 0.6);
        ...
        $fpdf->SetX(1);
        $fpdf->tabelbbcustom(array("$ppn->tax_name ($ppn->tax_cuts%)", "", "Rp " . rupiah($ppn->salary_tax_nominal)), 0.5);
        $fpdf->SetWidths(array(19.4));
        $fpdf->SetY(12);
        if (empty($salary->salary_date)) {
            $salary_date = "";
