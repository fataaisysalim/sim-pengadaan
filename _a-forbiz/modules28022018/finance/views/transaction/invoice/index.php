<?php if (in_array(1, array($permit->access_create, $permit->access_special, $permit->access_update))) { ?>
    ...
    <script type="text/javascript">
        $(".load_main_data").load("<?php echo base_url("$url"); ?>/form/<?php echo isset($invoice_id) ? $invoice_id : NULL ?>");
                                        
    </script>
<?php } else { ?>
    <h1 class="pull-left mg-t-sm"><i class="fa fa-warning mg-r-sm"></i></h1>
    <h1 class="pull-left mg-t-sm"><i class="fa fa-warning mg-r-sm"></i></h1><h3><small>Sorry you are not authorized to access the features</small><br/> <b>Invoice Transaction</b></h3>
    <?php } ?>
