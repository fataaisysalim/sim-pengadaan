				...
				<div class="col-xs-7 mg-b-sm">
					<div class="form-group">
                        ...
                        <select id="actor" name="actor" onchange="getActData()" class="form-control form-select2" data-style="btn-white" data-placeholder="Choose Supplier/Nasabah">
                                <option value="">-Pilih Supplier-</option>
                                <?php foreach ($supplier['ct'] as $nom => $ct) : ?>
                                     <option <?php if (isset($row->supplier_id) && $row->supplier_id == $ct->id) echo "selected"; ?> value="<?php echo $ct->id; ?>"><?php echo $ct->nama; ?></option>
                                <?php endforeach; ?>   
                            </select>
                    </div>
                </div>
...
<?php } ?>
	...
	/* function fill_bapb(){
		$("#no_bapb").val($("#spb").find("option:selected").attr("data-bapb"));
		//$('#no_bapb').val($("#spb option:selected").attr('data-bapb'));
	} */
	
	$("#spb").change(function(){
            var element = $("option:selected", this);
            var bapb = element.attr("data-bapb");
            $('#no_bapb').val(bapb);
    });
