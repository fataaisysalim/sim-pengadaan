...
class Equipment extends MY_Controller {
    ...
    private $url = "procurement/transaction/equipment/";
    private $view_path = "procurement/transaction/equipment/";
    private $view_path_entry = "procurement/transaction/equipment/entry/";
    private $view_path_exit = "procurement/transaction/equipment/exit/";
    private $view_path_return = "procurement/transaction/equipment/return/";
    ...
    public function out($id = null) {
        $data['sess'] = $this->authentication_root();
        $data['features'] = "warehouse";
        $data['trans_m'] = 1;
        $data['transExitEq_sm'] = 1;
        $data['title'] = $this->title;
        $data['url_access'] = $this->url;
        if ($data['sess']['position_id'] == 1) {
            if (!empty($id)) {
    ...
    public function returned($id = null) {
        ...
        $data['url_access'] = "$this->url";
        $data['url_action'] = $this->url . 'save/';
        $data['content'] = $this->view_path . "return/index";
        $this->load->view("../index", $data);
    }

    public function maintenance($ct = null) {
        $data['sess'] = $this->authentication_root();
        $data['features'] = "warehouse";
        $data['trans_m'] = 1;
        $data['transMaEq_sm'] = 1;
        ...
        $data['url_access'] = "$this->url";
        $data['url_action'] = $this->url . 'save/';
        $data['project'] = $this->crud_model->read_fordata(array("table" => "project p", "join" => array("equipment_stock_final esf" => "esf.project_id = p.project_id"), "group" => "p.project_id"))->result();
        $data['content'] = $this->view_path . "maintenance/index";
        $this->load->view("../index", $data);
    }
    ...
    public function info($start = null, $end = null, $ct = null, $project = null) {
        ...
        if (!empty($start) && !empty($end) && !empty($ct)) {
            ...
            $this->load->view("procurement/info/equipt_transaction/data", $data);
        } else {
            $data['project'] = $this->origin_model->proByAccess($data['sess']['users_id']);
            $data['show'] = $this->origin_model->transEq();
            $this->load->view("procurement/info/equipt_transaction/index", $data);
        }
    }
    ...
    public function process($ct = null, $project = null) {
        ...
        if (!empty($ct)) {
            ...
            $this->load->view("procurement/info/equipt_transaction/data_process", $data);
        } else {
            $data['project'] = $this->origin_model->proByAccess($data['sess']['users_id']);
            $data['show'] = $this->origin_model->transEq();
            $this->load->view("procurement/info/equipt_transaction/index_process", $data);
        }
    }
    ...
    public function detail_process($id) {
        ...
        $this->load->view("procurement/info/equipt_transaction/detail_process", $data);
    }
    ...
    public function detail($id) {
        ...
        $this->load->view("procurement/info/equipt_transaction/detail", $data);
    }
    ...
    public function form($feature = NULL, $id = NULL) {
        ...
        if (!empty($id)) {
//            $data["transaction_entry_dt"] = $this->crud_model->read_fordata(array("table" => "transaction_entry e", "where" => array("md5(transaction_entry_id)" => $id), "join" => array("transaction_entry_ct ec" => "ec.transaction_entry_ct_id = e.transaction_entry_ct_id")))->row();
        }

//        $data['actor'] = $this->origin_model->actor(1);
        ...
        if ($feature == 'entry') {
            if ($data['sess']['position_id'] == 5) {
                ...
                $data['equipt_trans'] = $this->crud_model->read_fordata(array("table" => "equipt_transaction et", "where" => array("equipt_transaction_status" => 0, "transaction_ct_id" => 1), "join" => array("actor a" => "a.actor_id = et.actor_id", "project p" => "p.project_id = et.project_id")))->result();
            } else if ($data['sess']['position_id'] == 1 && !empty($id)) {
                $data['equipt_transaction'] = $id;
            ...
            } elseif ($data['sess']['position_id'] == 1) {
                $data['code'] = $this->crud_model->read_fordata(array("table" => "code", "where" => array("code_ct_id" => 2, "code_status" => 1)))->result();
                $data['project'] = $this->crud_model->read_fordata(array("table" => "project p", "join" => array("project_access pa" => "pa.project_id = p.project_id"), "where" => array("md5(users_id)" => $data['sess']['users_id'])))->result();
                $data['equipment'] = $this->crud_model->read_fordata(array("table" => "equipment", "where" => array("equipment_status" => 1)))->result();
                $data['equipt_trans'] = $this->crud_model->read_fordata(array("table" => "equipt_transaction et", "where" => array("equipt_transaction_status" => 0, "transaction_ct_id" => 1), "join" => array("actor a" => "a.actor_id = et.actor_id", "project p" => "p.project_id = et.project_id")))->result();
            ...
            $data['code'] = $this->crud_model->read_fordata(array("table" => "code", "where" => array("code_ct_id" => 2, "code_status" => 1)))->result();
            $data['project'] = $this->crud_model->read_fordata(array("table" => "project p", "join" => array("project_access pa" => "pa.project_id = p.project_id"), "where" => array("md5(users_id)" => $data['sess']['users_id'])))->result();
            if ($data['sess']['position_id'] == 1 && !empty($id)) {
                $data['feature'] = 'exit';
    ...
    public function equipment_by_project($id_project = NULL, $actor = NULL, $counter) {
        $data['sess'] = $this->authentication_root();
    ...
    public function get_stok($id = NULL, $project_id = NULL) {
        $data = $this->crud_model->read_fordata(array("table" => "equipment e", "join" => array("equipment_unit eu" => "eu.equipment_unit_id = e.equipment_unit_id", "equipment_stock_final esf" => "esf.equipment_id = e.equipment_id"), "where" => array("e.equipment_id" => $id, "esf.project_id" => $project_id, "e.equipment_status" => 1)))->row();
        echo json_encode(array('status' => 1, 'data' => $data));
    }
    ...
    public function save($id = NULL) {
        ...
        $date = NULL;
        if (!empty($id) || $this->input->post('equipt_transaction_dt')) {
...
//        $equipt['equipt_transaction_date_verify'] = $this->input->post("equipt_transaction_date_verify") ? date('Y-m-d H:i:s', strtotime($this->input->post("equipt_transaction_date_verify"))) : NULL;
        ...
        if (!empty($id)) {
            ...
            $sts =  $data['sess']['position_id'] == 1 || $price ? 1 : 0;
        }
...
//                $where['equipt_transaction_letter'] = $equipt['equipt_transaction_letter'];
                ...
                if ($this->input->post("equipt_transaction")) {
                    ...
                    if ($data['sess']['position_id'] == 1 OR $data['sess']['position_id'] == 5) {
                        $equipt['equipt_transaction_status'] = 1;
                        $equipt['equipt_transaction_date_verify'] = date('Y-m-d H:i:s');
                    }
                    $getTr = $this->crud_model->read_fordata(array("table" => "equipt_transaction et","join"=>array("actor a"=>"a.actor_id = et.actor_id"), "where" => array("md5(et.equipt_transaction_id)" => $this->input->post("equipt_transaction"))))->row();
                    $updt['equipt_transaction_id'] = $getTr->equipt_transaction_id;
                    $updt['equipt_transaction_status'] = 1;
                    $this->crud_model->update_data('equipt_transaction', $updt, 'equipt_transaction_id');
                    $trct = $getTr->transaction_ct_id == 1?"BAPP":"BPP";
                    $this->recActivity("Updated $trct <b>No. $getTr->equipt_transaction_number by $getTr->actor_name</b> on $trct process", "procurement");
                } else {
...
//        $this->session->set_flashdata("msgTransEq", "<div class='alert alert-success alert-dismissable'><button type='button' class='close' data-dismiss='alert' aria-hidden='true'>&times;</button><i class='fa fa-info-circle mg-r-md'></i> Data successfully saved</div>");
        if($equipt['transaction_ct_id'] == 1) :
    ...
    public function get_equipment($project = null, $counter = null) {
        ...
        $this->load->view("procurement/transaction/equipment/extend/equipment", $data);
    }

    public function equipment_entry($id = null) {
        ...
        $this->load->view("procurement/pdf/form_masuk_alat", $data);
    }

    public function equipment_exit($id = null) {
        ...
        $data['detail'] = $this->crud_model->read_data('apps',null,null,null)->row();
        $this->load->view("procurement/pdf/form_keluar_alat", $data);
    }
    
    public function delete($id = NULL) {
        if(!empty($id)) {
            $get_etrans = $this->crud_model->read_fordata(array("table" => "equipt_transaction", "where" => array("md5(equipt_transaction_id)" => $id)))->row();
