    ...
    <div class="panel-body">
        ...
        <div class="row">
            ...
            <div class="col-lg-6 col-md-6 col-sm-6 col-xs-6">
                <div class="form-group">
                    <label>Role</label>
                    <select name="users_divisi" class="form-control form-select2" data-placeholder="Choose Position" data-style="btn-white">
                        <option value=""></option>
                        <?php foreach ($users_role as $up) : ?>
                            <option value="<?php echo $up->id_user_role; ?>" <?php if (!empty($users_dt) && $up->id_user_role == $users_dt->users_divisi) echo "selected"; ?> <?php if ($up->id_user_role == set_value('users_divisi')) echo "selected"; ?>><?php echo $up->nama_user_role; ?></option>
                        <?php endforeach; ?>
                    </select>
                </div>
            </div>
...
<?php } ?>
        ...
        $("#form").submit(function() {
            ...
            if ($("select[name=users_position").val() == '') {
                ...
                $("select[name=users_position").focus();
                return false;
            }
            if ($("select[name=users_divisi").val() == '') {
                bootbox.alert("<i class='fa fa-warning mg-r-sm'></i>Role can not be empty");
                $("select[name=users_divisi").focus();
                return false;
            }
