...
<!--        <div class="form-group">
        ...
        <?php //} else { ?>
        <input type="hidden" id="status_<?php echo $no; ?>" name="status[]" value="<?php echo $row->equipt_transaction_dt_status; ?>" class="form-control">
        <input type="text" id="status_name<?php echo $no; ?>" value="<?php echo $row->equipt_transaction_dt_status == 1 ? "Beli" : "Sewa"; ?>" class="form-control" readonly>
        <?php //} ?>
    </td>
    <td class="hidden-xs">
        <input readonly type="text" id="condition_<?php echo $no; ?>" name="condition[]" value="<?php echo $row->equipt_transaction_dt_condition; ?>" class="form-control" placeholder="Kondisi">
    </td>
    <td>
        <input readonly id="volume_<?php echo $no; ?>" class=" form-control " onkeyup="get_count(this, <?php echo $no; ?>, 'volume')" name="volume[]" value="<?php echo isset($row->equipt_transaction_dt_volume) ? rupiah($row->equipt_transaction_dt_volume) : NULL; ?>" class="form-control" placeholder="Volume" >
    </td>
    <?php //if($equipt_transaction_status == 1 || $sess['position_id'] == 5) { ?>
    <td>
    ...
    <td class="hidden-xs total_sub" id="total_sub_<?php echo $no; ?>">
        <?php echo isset($row->equipt_transaction_dt_price) ? rupiah($row->equipt_transaction_dt_price * $row->equipt_transaction_dt_volume) : NULL; ?>
    </td>
    <?php //} ?>
