            ...
            <div class="col-sm-5">
                ...
                <div class="col-xs-5 mg-b-md">
                    <div class="form-group">
                        ...
                        <input type="text" name="equipt_transaction_letter" class="form-control" value="<?php echo isset($equipt_trans_dt) ? $equipt_trans_dt->equipt_transaction_letter : NULL; ?>" placeholder="Travel Doc"/>
                    </div>
                </div>
                <div class="col-xs-6 mg-b-md">
                    <div class="form-group">
                        <label>V. License Plate</label>
                        <input type="text" name="equipt_transaction_car" class="form-control" value="<?php echo isset($equipt_trans_dt) ? $equipt_trans_dt->equipt_transaction_car : NULL; ?>" placeholder="V. License Plate"/>
                    </div>
                </div>
                <div class="col-xs-6 mg-b-md">
                    <div class="form-group">
                        <label>Driver</label>
                        <input type="text" name="equipt_transaction_driver" class="form-control" value="<?php echo isset($equipt_trans_dt) ? $equipt_trans_dt->equipt_transaction_driver : NULL; ?>" placeholder="Driver"/>
                    </div>
                </div>
                <div class="col-xs-6 mg-b-md">
                    <div class="form-group">
                        <label>KTP/SIM</label>
                        <input type="text" onkeypress="return isNumber(event)" name="equipt_transaction_driver_identity" class="form-control" value="<?php echo isset($equipt_trans_dt) ? $equipt_trans_dt->equipt_transaction_driver_identity : NULL; ?>" placeholder="KTP/SIM"/>
                    </div>
                </div>
            ...
            <div class="panel-body row">
                <div class="row">
                    <table class="table table-bordered table-striped" style="margin-top: -13px">
                        ...
                        <tr>
                            <td colspan="7">
                                <button class="btn btn-md btn-dark col-xs-12" onclick="adds_row()" type="button"><i class="fa fa-plus"></i></button>
                            </td>
                            <?php if ($permit->access_special == 1) { ?>
            ...
            <div class="col-xs-6 col-xs-offset-6">
                <?php if (!empty($equipt_trans_dt)) { ?>
                    <div class="<?php echo!empty($equipt_trans_dt) ? 'col-xs-6' : null ?>">
                        <a href="<?php echo base_url() ?>warehouse/bapp" role="button" style="cursor: pointer" class="<?php echo empty($equipt_trans) ? 'cancelEq' : null ?> btn btn-danger col-xs-12"><i class="fa fa-times mg-r-sm"></i> Cancel </a>
                    </div>
                <?php } ?>
...
<?php if (empty($equipt_trans_dt)) { ?>
            $.ajax({
                url: "<?php echo base_url('warehouse/get-project'); ?>/" + $('#project').val() + '/1/equipt_transaction',
                dataType: "JSON",
...
<?php if ($permit->access_update == 1) { ?>
    <?php if (isset($equipt_transaction)) { ?>
                $('#show_row').load("<?php echo base_url() . 'warehouse/bapp/trans/detail/' . $equipt_transaction; ?>");
    <?php } ?>
<?php } ?>
        $(".cancelEq").click(function () {
            $(".load_main_data").load("<?php echo base_url() ?> warehouse/bapp/form");
        });
    });
    ...
    function adds_row() {
        ...
        var baris = '<tr class="row_out row_tam">'
                ...
                + '<input type="hidden" name="equipt_transaction_dt[]" value="">'
<?php if ($permit->access_special == 1) { ?>
...
<?php } ?>
        + '<td id="equipment_' + counter + '"></td>'
                ...
                + '</td>'
                + '<td class="hidden-xs">'
                + '<input type="text" maxlength="20" id="condition_' + counter + '" name="condition[]" value="" class="form-control" placeholder="Condition">'
                + '</td>'
                + '<td>'
...
<?php } ?>
        ...
        $('#equipment_' + counter).load("<?php echo base_url() ?>warehouse/e-trans/data-equipment/all/" + counter);
    }

    function get_unit(i, ii) {
        $.ajax({
            url: "<?php echo base_url('warehouse/e-trans/data-unit'); ?>/" + i.value,
            dataType: "JSON",
            success: function (json) {
                if (json.status == 1) {
    ...
    function get_detail(el) {
        $.ajax({
            url: "<?php echo base_url() . 'warehouse/bapp/trans/'; ?>" + el.value,
            dataType: "JSON",
            success: function (json) {
                if (json.status == 1) {
                    ...
                    $('#show_row').load("<?php echo base_url() . 'warehouse/bapp/trans/detail/'; ?>" + el.value);
                }
            }
        });
        return false;
        ...
        $.ajax({
            ...
            success: function (json) {
                if (json.status == 0) {
                    ...
                    window.open("<?php echo base_url() ?>warehouse/bapp-print/" + json.id);
                    window.location.href = "<?php echo base_url() ?>warehouse/bapp";
                }
            }
        });
        return false;
