<script>
    ...
    $('input[type="radio"]').on('click', function(e) {
        ...
        <?php if ($this->uri->segment(5) === "all") { ?>
            getTabs();
        <?php }else{?>
            getTabsById(<?php echo $sess['users']->users_divisi; ?>);
        //    alert('aaa');
        <?php }?>
    ...
    var availableTags = [
      ...
      "Scheme"
        ...
        $( "#supplier" ).autocomplete({
          ...
          change: function (event, ui) { getTabs(); }
        });
...
<section class="panel panel-info">
    ...
    <!--     <div class="form-group">  
                 ...
                 <div class="row">
                    <div class="col-xs-3 mg-b-md">
                        ...
                        <div class="input-group mg-b-md">
                            <input type="radio" name="jenis" id="jenis" value="pengajuan"> Pengajuan
            ...
            <div class="col-xs-1 mg-b-md">
                <div class="form-group">
                <label>&nbsp;<span style="color:white">blank</span></label>
				<span>
				<?php if ($this->uri->segment(5) === "all") { ?>
                    <input type="button" id="mog_number" name="mog_number" onclick="getTabs()" class="btn btn-success" value="&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Filter&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;">
				<?php } else { ?>
					<input type="button" id="mog_number" name="mog_number" onclick="getTabsById(<?php echo $sess['users']->users_divisi; ?>)" class="btn btn-success" value="&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Filter&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;">
				<?php } ?>	
				</span>
				</div>
            </div>
                ...
                <table class="table table-bordered table-striped datatable" style="margin-top: -13px">
                    ...
                    <tbody id="filterTabs">
                       ...
                       <tr>
                        ...
                        <td class="text-center" style="width: 50px; padding: 15px"><?php echo $row->tujuan; ?>
                        </td>
                        <?php  
                        $view="Approval";
                        ...
                        <?php   if($this->uri->segment(5)!=""){ 
                                    if($this->uri->segment(5)=='all'){
                                        if($dtx['users']->users_divisi==1 or $dtx['users']->users_divisi==16){
                                        if(($row->role_id==11 or $row->role_id==16)){
                        ?>
                                        <?php if($row->stat_edit==''){?>
                        ...
                        <!--a class="btn btn-success" onclick="reqEdit('<?php echo $idMog;?>')"><i class="fa fa-edit"></i>Req Edit</a>
                        <a class="btn btn-success" onclick="reqDelete('<?php echo $idMog;?>')"><i class="fa fa-edit"></i>Req Delete</a-->
                            <?php if($sess['users']->users_divisi == 11 || $sess['users']->users_divisi == 12 || $sess['users']->users_divisi == 13 || $sess['users']->users_divisi == 7 || $sess['users']->users_divisi == 10 || $sess['users']->users_divisi == 18 || $sess['users']->users_divisi == 17 || $sess['users']->users_divisi == 19 || $sess['users']->users_divisi == 20 ) {?>
                                <?php if($row->role_id != 100) {?>
                                    <a class="btn btn-success" href="<?php echo $link_view?>"><i class="fa fa-edit"></i><?php echo $view?></a>
                                <?php } ?>                            
                            <?php } ?>
                            <?php 
                            ...
                            if ($sess['users']->users_divisi == 11 or $sess['users']->users_divisi == 13){ ?>
                        ...
                        <?php  if($sess['users']->users_divisi == 1 || $sess['users']->users_divisi == 17){ ?>
                            ...
                            <?php } ?>
                        <?php }?>
                        <?php if ($sess['users']->users_divisi == 11 or $sess['users']->users_divisi == 13) { ?>
                        ...
                        <?php if (!empty($row->no_bapb)){ ?>
                        <a class="btn btn-success" href="<?php echo base_url() ?>procurement/transaction/material/cetak_bapb/<?php echo $row->mog_id?>" target="_blank"><i class="fa fa-print"></i>Cetak BAPB</a>
                        <?php } ?>
...
<script type="text/javascript">
    function getTabs(){
        ...
        $.ajax({
            ...
            beforeSend: function() {
              $("#filterTabs").html("<div class='loading_div'><img src='<?php echo base_url() ?>assets/img/ajax-loader.gif'></div>");
            },
...
<script type="text/javascript">
    function getTabsById(id){
        ...
        $.ajax({
            url : "<?php echo site_url('procurement/transaction/material/filter')?>",
            type: "POST",
            data: {"noPengajuan":noPengajuan, "tglAwal":tglAwal, "tglAkhir":tglAkhir, "jenis":jenis, "supplier":supplier, "jenis_material":jenis_material, "volume":volume, "operator":operator, "id":id},
            beforeSend: function() {
              $("#filterTabs").html("<div class='loading_div'><img src='<?php echo base_url() ?>assets/img/ajax-loader.gif'></div>");
            },
            success: function(data){
...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        $("#mog_numberstart").change(function() {
             <?php if ($this->uri->segment(5) === "all") { ?>
                getTabs();
            <?php }else{?>
                getTabsById(<?php echo $sess['users']->users_divisi; ?>);
