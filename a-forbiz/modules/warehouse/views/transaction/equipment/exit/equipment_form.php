            ...
            <div class="col-sm-6">
                ...
                <div class="col-xs-6 mg-b-md">
                    <div class="form-group">
                        ...
                        <input maxlength="11" type="text" name="equipt_transaction_car" class="form-control" value="<?php echo isset($equipt_trans) ? $equipt_trans->equipt_transaction_car : NULL; ?>" placeholder="V. License Plate"/>
                    </div>
                </div>
                <div class="col-xs-6 mg-b-md">
                    <div class="form-group">
                        <label>Driver</label>
                        <input type="text" name="equipt_transaction_driver" class="form-control" value="<?php echo isset($equipt_trans) ? $equipt_trans->equipt_transaction_driver : NULL; ?>" placeholder="Diver"/>
                    </div>
                </div>
                <div class="col-xs-6 mg-b-md">
                    <div class="form-group">
                        <label>KTP/SIM</label>
                        <input type="text" onkeypress="return isNumber(event)" name="equipt_transaction_driver_identity" class="form-control" value="<?php echo isset($equipt_trans) ? $equipt_trans->equipt_transaction_driver_identity : NULL; ?>" placeholder="KTP/SIM"/>
                    </div>
                </div>
...
<!--                                <th class="text-center" style="width: 120px; padding: 15px">Harga</th>
        ...
        <div class="row">
            <?php if (!empty($equipt_trans)) { ?>
                <div class="col-xs-6">
                    <a href="<?php echo base_url() ?>warehouse/bpp" role="button" style="cursor: pointer" class="<?php echo empty($equipt_trans) ? 'cancelEq' : null ?> btn btn-danger col-xs-12"><i class="fa fa-times mg-r-sm"></i> Cancel </a>
                </div>
            <?php } ?>
            <div class="<?php echo!empty($equipt_trans) ? 'col-xs-6' : 'col-md-6 col-sm-12 col-xs-6' ?> pull-right">
...
<?php if (empty($equipt_trans)) { ?>
            $.ajax({
                url: "<?php echo base_url('warehouse/get-project'); ?>/" + $('#project').val() + '/2/equipt_transaction',
                dataType: "JSON",
...
<?php if ($permit->access_update == 1 && !empty($equipt_trans)) { ?>
            $('#show_row').load("<?php echo base_url() . 'warehouse/bpp/trans/detail/' . md5($equipt_trans->equipt_transaction_id) . '/' . md5($equipt_trans->project_id); ?>");
<?php } ?>
    });
    ...
    function adds_row() {
        ...
        $('#td_equipment_' + counter).load("<?php echo base_url(); ?>warehouse/e-trans/data-by-project/" + $("select[name=project] option:selected").val() + "/" + $("select[name=actor] option:selected").val() + "/" + counter);
    }
    ...
    function get_unit(i, ii) {
        $.ajax({
            url: "<?php echo base_url('warehouse/e-trans/data-stock'); ?>/" + i.value + "/" + $("select[name=actor] option:selected").val() + "/" + $("#project").val(),
            dataType: "JSON",
            success: function (json) {
                if (json.status == 1) {
    ...
    $("#form").submit(function () {
        ...
        for (var e = 1; e <= jmlTransM; e++) {
            ...
            if (qty != '') {
                if (equipment == '') {
                    bootbox.alert("<i class='fa fa-warning mg-r-sm'></i>Equipment ordered can not be empty");
                    return false;
                }
...
<?php } ?>
            ...
            if (qty != '') {
                ...
                if ($("#status_volume_" + e).val() == '3') {
                    ...
                    var status_tr = $("#data_status_" + e).val();
                    if (status_tr != '') {
                        if (status_tr == '3') {
                            if (currencyToNumber(qty) > parseInt($("#stok_" + e).val())) {
                                bootbox.alert("<i class='fa fa-warning mg-r-sm'></i>Stock insufficient. Remaining stock " + numberToCurrency($("#stok_" + e).val()));
        ...
        $.ajax({
            ...
            success: function (json) {
                if (json.status == 0) {
                    ...
                    window.open("<?php echo base_url() ?>warehouse/bpp-print/" + json.id);
                    window.location.href = "<?php echo base_url() ?>warehouse/bpp";
                }
            }
        });
        return false;
