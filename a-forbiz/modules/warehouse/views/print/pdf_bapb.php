...
$fpdf->tabelbb(array("BERITA ACARA PENERIMAAN BARANG"), 0.5);
$fpdf->SetX(5);
$fpdf->SetFont("ARIAL", "B", 8);
$fpdf->SetWidths(array(3, 2.95, 0.6, 1.4));
...
$fpdf->SetX(5);
$fpdf->tabelbb(array("Leveransir", " $mogdet->actor_name"), 0.25);
$fpdf->SetX(5);
$fpdf->tabelbb(array("Surat Jalan", " $mogdet->mog_number_letter"), 0.5);
$fpdf->SetX(5);
$fpdf->tabelbb(array("Tanggal", " " . indo_date($mogdet->mog_date)), 0.35);
$fpdf->SetXY(13.3, 2.7);
$fpdf->tabelbb(array("S.P.B", " "), 0.45);
$fpdf->SetX(13.3);
$fpdf->tabelBB(array("S.P.K", " "), 0.4);
$fpdf->SetXY(7.5, 4);
$fpdf->SetFont("ARIAL", "", 8);
$fpdf->Ln(0.2);
$fpdf->SetWidths(array(1, 2.5, 4.25, 1.5, 1, 2.4, 3, 4.05));
$fpdf->SetFont("ARIAL", "B", 9);
$fpdf->tabelc(array("NO", "KODE S. DAYA", "JENIS MATERIAL", "SAT", 'VOL', "H. SATUAN", "JUMLAH", "KETERANGAN"), 0.5);
...
foreach ($mog as $i => $row):
    if ($row->mog_dt_convertion == null or $row->mog_dt_convertion == 0) {
        ...
        $totl = $row->mog_dt_volume * $row->mog_dt_price;
    } else {
        $volume = $row->mog_dt_volume;
        $totl = ($row->mog_dt_volume * $row->mog_dt_convertion) * $row->mog_dt_price;
    }
    $totall += $totl;
    $fpdf->tabelc(array(++$i, "$row->code_name", strtoupper($row->material_sub_name), "$row->material_unit_name", rupiah($volume), $sess['position_id'] == 1 ?rupiah($row->mog_dt_price):"", $sess['position_id'] == 1 ? rupiah($totl) : "", "$row->mog_dt_note"), 0.4);
    $toti += $i;
    if($i % 5==0){
        if ($toti <= 5) {
    $f = 5 - $toti;
    for ($j = 0; $j <= $f; $j++) {
        $fpdf->tabelc(array("", "", "", "", '', "", "", ""), 0.4);
    }
}
if($i-count($mog)!=0){
    $y = $fpdf->GetY();
$fpdf->SetWidths(array(2, 2.9));
$fpdf->SetX(10.55);
$fpdf->tabelbbr(array("JUMLAH", $sess['position_id'] == 1 ? rupiah($totall) : ""), 0.45);
$fpdf->SetWidths(array(1.65));
$fpdf->SetXY(0.3, 9.7);
$fpdf->tabelbb(array("CATATAN"), 0.4);
$fpdf->SetWidths(array(1.65, 4));
$fpdf->SetX(0.8);
$fpdf->tabelbb(array("1 - 5 & 8", ": Di isi oleh Bag. Gudang"), 0.4);
$fpdf->SetX(0.8);
$fpdf->tabelbb(array("6 - 7", ": Di isi oleh Bag. KSPP"), 0.4);
$fpdf->Ln(0);
$fpdf->SetWidths(array(1.1, 5.6));
$fpdf->SetFont("ARIAL", "B", 9);
$fpdf->SetWidths(array(5, 5, 5, 5));
$fpdf->tabelbbc(array("KSPP", "KSKA", "QC", "GUDANG"), 0.5);
$fpdf->Ln(1);
$fpdf->tabelbbc(array("( .......................... )", "( .......................... )", "( .......................... )", "( .......................... )"), 0.1);
$fpdf->SetFont("ARIAL", "", 7);
$fpdf->SetX(1.4);
$fpdf->tabelbb(array("* Harap dilampirkan pada saat penagihan"), 0.9);
//$fpdf->Rect(0.2, 0.6, 19.69, 13);
//$fpdf->Rect(0.2, 0.6, 3.9, 1.3);
$fpdf->Rect(16, 0.6, 3, 2);
...
$fpdf->Rect(13.3, 2.7, 5.9, 0.4);
$fpdf->Rect(13.3, 3.1, 5.9, 0.4);
$fpdf->Rect(13, $y, 3, 0.4);
$fpdf->SetFont("ARIAL", "B", 8);
$fpdf->SetXY(16.3, 0.75);
$fpdf->tabelbb(array("1. Supplier"), 0.35);
$fpdf->SetX(16.3);
$fpdf->tabelbb(array("2. KSPP"), 0.35);
$fpdf->SetX(16.3);
$fpdf->tabelbb(array("3. Gudang"), 0.35);
$fpdf->SetX(16.3);
$fpdf->tabelbb(array("4. QC"), 0.35);
$fpdf->SetX(16.3);
$fpdf->tabelbb(array("5. KSKA"), 0.35);
