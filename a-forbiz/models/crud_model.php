...
class Crud_model extends CI_Model {
    ...
    public function getNameProject($idProject) {
     ...
     public function filterTabs($noPengajuan,$tglAwal,$tglAkhir, $jenis, $supplier, $jenis_material,$volume, $operator,$id) {
        ...
        $sql="SELECT * from mog 
            ...
            LEFT JOIN actor ON actor.actor_id = mog.actor_id
            
            where 1=1 ";
        if(!empty($noPengajuan)){
            $sql.=" AND nomor_pengajuan like '%$noPengajuan%'"   ;
        }
        if(!empty($jenis)){
            $sql.=" AND jenis_procurement like '%$jenis%'"   ;
        }
        if(!empty($supplier)){
            $sql.=" AND actor_name like '%$supplier%'"   ;
        }
        if(!empty($jenis_material)){
            //$sql.=" OR material_sub_name like '%$jenis_material%'"   ;
            $sql.=" AND mog.mog_id IN (SELECT mog_id FROM mog_dt 
                INNER JOIN material_sub ON material_sub.material_sub_id = mog_dt.material_sub_id
        ...
        if($tglAwal!=null){
            $sql.=" AND tanggal_spb BETWEEN '$tglAwal' AND '$tglAkhir'";
        }
        ...
        if(!empty($volume)){
            ...
            $sql.=" AND mog.mog_id IN (SELECT mog_id FROM mog_dt 
                INNER JOIN material_sub ON material_sub.material_sub_id = mog_dt.material_sub_id
                GROUP BY mog_id
                HAVING SUM(mog_dt_volume) $operator '$volume'
                )"   ;
        }
...
//        }
    ...
    public function getLastNoSpb(){
        $q = $this->db->query("select MAX(RIGHT(no_spb,5)) as kd_max from mog");
        $kd = "";
        if($q->num_rows()>0){
            foreach($q->result() as $k){
                $tmp = ((int)$k->kd_max)+1;
     ...
     public function getLastNoPengajuan(){
        $q = $this->db->query("select MAX(SUBSTRING(nomor_pengajuan,1,4)) as kd_max from mog");
        $kd = "";
        if($q->num_rows()>0){
            foreach($q->result() as $k){
                $tmp = ((int)$k->kd_max)+1;
    ...
    public function get_total_pengajuan($no_kontrak){
        $query = "
            SELECT SUM(mog_total) AS total_pengajuan
            FROM mog
            WHERE no_kontrak = ".$this->db->escape($no_kontrak)."
            AND jenis_procurement = 'pengajuan'
        ";

        return $this->db->query($query)->row();
    }

    public function get_budget_amount($no_kontrak){
        $query = "
            SELECT SUM(mog_total) AS budget_amount
            FROM mog
            WHERE no_kontrak = ".$this->db->escape($no_kontrak)."
            AND jenis_procurement = 'pengadaan'
        ";
