...
class Kirim_barang extends MY_Controller {

    private $title = "Kirim Barang";
    private $url = "procurement/kirim_barang/";
    ...
    public function index($id = null) {
        ...
        //}
		$data['idne'] ="";
        $data['content'] = "procurement/transaction/kirim_barang/index";
    ...
    public function form($id = NULL) {
        ...
        //$data['transaction_ct'] = "Material Receipt (BAPB)";
        $data['transaction_ct'] = "Penerimaan Material (BAPB)";
        $data['url_access'] = "$this->url";
        ...
        //if ($data['permit']->access_update == 1) {
            if (!empty($id)) {
                $data['mog_id'] = $id;
                $data['mog_dt'] = $this->crud_model->read_fordata(array("table" => "mog", "join" => array("project p" => "p.project_id = mog.project_id", "actor a" => "a.actor_id = mog.actor_id"), "where" => array("mog_id" => $id)))->row();
            }
        //}
        $data['material_sub'] = $this->crud_model->read_fordata(array("table" => "material_sub", "where" => array("material_sub_status" => 1)))->result();

        $this->load->view("procurement/transaction/kirim_barang/form", $data);
    }
    ...
    public function get_mog_detail($id = NULL) {
        if (!empty($id)) {
            ...
            $this->load->view("warehouse/transaction/material/extend/mog_entry_detail", $data);
        }
    }
    ...
    public function save($id = NULL) {
        ...
        if (isset($mog)) {
            ...
            if (empty($id)) {
                ...
                if ($duplicate == 0) {
                    $mog['mog_status'] = 0;
                    $mog['mog_date'] = date("Y-m-d H:i:s",strtotime($this->input->post("mog_date")));
                    if ($data['permit']->access_special == 1) {
                        $mog['mog_status'] = 1;
                        $mog['mog_date_verify'] = date('Y-m-d H:i:s');
                    }
                    $this->crud_model->insert_data("mog", $mog);
                    $this->recActivity("Membuat BAPB No. <b>$mog[mog_number] (<i>$actor->actor_name</i>)</b> on Transaction BAPB", "warehouse");
                    $actions = "tambah";
                ...
                $this->recActivity("Memperbarui BAPB No. <b>$mog[mog_number] (<i>$actor->actor_name</i>)</b> on Transaction BAPB", "warehouse");
                $actions = "edit";
        ...
        for ($in = 0; $in < count($material_sub); $in++) {
            ...
            if (isset($material_sub[$in]) || isset($mog_dt_volume[$in])) {
                ...
                $mog_dt[$action[$in]]['mog_dt_date'] = date("Y-m-d H:i:s",strtotime($this->input->post("mog_date")));
                if ($action[$in] == 'edit') {
                ...
                if (in_array(1, array($data['permit']->access_special, $data['permit']->access_create))) {
...
//                    $stock['material_sub_id'] = $material_sub[$in];
    ...
    public function delete($id = NULL) {
        if (!empty($id)) {
            ...
            $sts = $get_mog->transaction_ct_id == 1 ? "BAPB" : "BPM";
            $this->recActivity("menghapus $sts No. <b>$get_mog->mog_number (<i>$get_mog->actor_name</i>)</b> of BAPB Information", "warehouse");
            $this->crud_model->delete_data("stock", $data);
            $this->crud_model->delete_data("mog_dt", $data);
            $this->crud_model->delete_data("mog", $data);

            $this->session->set_flashdata("message", "<div class='alert alert-success alert-dismissable'><button type='button' class='close' data-dismiss='alert' aria-hidden='true'>&times;</button><i class='fa fa-info-circle mg-r-md'></i> Material successfully removed</div>");
            echo json_encode(array("status" => 1));
        }
    }
	
	public function get_supplier() {
		$get_data = $this->crud_model->read_data("mst_nasabah_konstruksi")->result();
		//$get_data = $this->db->select('id, nama, kode, npwp')->get("mst_nasabah_konstruksi")->result();
		$result = array();
	...
	/* public function getdata_suplier() {
		$get_data = $this->crud_model->read_data("mst_nasabah_konstruksi")->result();
		//$get_data = $this->db->select('id, nama, kode, npwp')->get("mst_nasabah_konstruksi")->result();
		$result = array();
		foreach($get_data as $row){
	...
	public function getdata_spbsuplier($supplier_id) {
        $data = $this->crud_model->read_data("procurement", array("supplier_id" => $supplier_id))->result();
		//$data = $this->db->query("SELECT *, pd.* FROM procurement p RIGHT JOIN procurement_detail pd ON p.id = pd.procurement_id WHERE p.supplier_id = '".$supplier_id."'")->result();
		echo json_encode(array('status' => 1, 'data' => $data));
    }
}
