    ...
    <div class="row">
        ...
        <div class="col-md-12">
            ...
            <div class="row mg-t-md mg-b-sm">
                ...
                <div class="col-sm-3 col-xs-6">
                    <i class="fa fa-print mg-r-md btn btn-sm btn-dark mg-b-sm" disabled></i> Print Disposition
                </div>
...
<script type="text/javascript">
    ...
    $(".modal-title").html('<a onclick="exportDoc()" role="button" class="btn btn-sm btn-dark pull-left" style="margin-top: -3px"><i class="fa fa-download mg-r-sm"></i>Excel</a><i class="fa fa-th-large mg-r-md"></i>Document IN Information');
    $(".docData").load("<?php echo base_url() ?>secretariat/doc_entry/data/" + $('select[name=projectsDoc] option:selected').val() + "/" + $('.startDoc').val() + "/" + $('.endDoc').val()+ "/" + $('select[name=docode] option:selected').val());
    ...
    function detailDocE(id) {
        $("#modal-contents").html('');
        $("#modal-contents").load("<?php echo base_url() ?>secretariat/doc_entry/detail/"+id);
        $(".modal-title").html('<div class="loader mg-t"><i class="fa fa-spin fa-refresh mg-r-md"></i>Loading data. Please wait...');
    }
    function exportDoc(){
        window.open("<?php echo base_url() ?>secretariat/doc_entry/export/" + $('select[name=projectsDoc] option:selected').val() + "/" + $('.startDoc').val() + "/" + $('.endDoc').val()+ "/" + $('select[name=docode] option:selected').val());
    }
<?php if ($permit->access_delete == 1) { ?>
