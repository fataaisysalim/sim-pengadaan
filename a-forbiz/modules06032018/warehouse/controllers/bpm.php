<?php

if (!defined('BASEPATH'))
    exit('No direct access allowed');

class Bpm extends MY_Controller {

    private $title = "Material Usage";
    private $url = "warehouse/bpm/";

    public function __construct() {
        parent::__construct();
        is_logged_in();
        $this->load->model(array("origin_model"));
    }

    public function index($id = null) {
        $data['sess'] = $this->authentication_root();
        $data['active'] = empty($id) ? $data['sess']['gotcurrent']->mod_menu_id : null;
        $data['permit'] = $data['sess']['permit'];
        $data['title'] = $this->title;
        $data['url_access'] = $this->url;
        if ($data['permit']->access_update == 1 && !empty($id)) {
            $data['mog_id'] = $id;
        }
        $data['content'] = "warehouse/transaction/material/exit/index";
        $this->load->view("../index", $data);
    }

    public function form($id = NULL) {
        $data['sess'] = $this->authentication_root();
        $data['permit'] = $data['sess']['permit'];
        $data['title'] = $this->title;
        $data['act'] = (empty($id)) ? "Tambah" : "Edit";
        $data['transaction_ct'] = "Pemakaian Material (BPM) ";
        $data['url_access'] = "$this->url";
        $data['url_action'] = $this->url . 'save/' . $id;
        $data['mandor']['ct'] = $this->crud_model->read_fordata(array("table" => "actor_category", "where" => array("actor_category_id" => 2), "or_where" => array("actor_category_id" => 3)))->result();
        foreach ($data['mandor']['ct'] as $i => $r) {
            $data['mandor']['act'][$i] = $this->crud_model->read_fordata(array("table" => "actor", "where" => array("actor_category_id" => $r->actor_category_id, "actor_status" => 1), "order" => array("actor_id" => "desc")))->result();
        }
        $data['code'] = $this->crud_model->read_fordata(array("table" => "code", "where" => array("code_ct_id" => 1)))->result();
        $data['project'] = $this->crud_model->read_fordata(array("table" => "project p", "join" => array("project_access pa" => "pa.project_id = p.project_id"), "where" => array("md5(users_id)" => $data['sess']['users_id'])))->result();
        $data['material_sub'] = $this->crud_model->read_fordata(array("table" => "material_sub ms", "join" => array("stock_final sf" => "sf.material_sub_id = ms.material_sub_id"), "group" => "ms.material_sub_id"))->result();
        if (!empty($id)) {
            $data["mog"] = $this->crud_model->read_fordata(array("table" => "mog", "where" => array("md5(mog_id)" => $id), "join" => array("project p" => "p.project_id = mog.project_id", "actor a" => "a.actor_id = mog.actor_id")))->row();
        }

        $this->load->view("warehouse/transaction/material/exit/form", $data);
    }

    public function material($project = null, $counter = null) {
        $data['count'] = $counter;
        $data['show'] = $this->origin_model->material_stock($project);
        $this->load->view("warehouse/transaction/material/extend/material", $data);
    }

    public function get_detail($id, $project_id) {
        $data['sess'] = $this->authentication_root();
        $data['permit'] = $data['sess']['permit'];
        if ($data['permit']->access_update == 1 && !empty($id)) {
            $data['code'] = $this->crud_model->read_fordata(array("table" => "code", "where" => array("code_ct_id" => 1)))->result();
            $data['mog_dt'] = $this->crud_model->read_fordata(array("table" => "mog_dt md", "join" => array("material_sub ms" => "ms.material_sub_id = md.material_sub_id", "material_unit mu" => "mu.material_unit_id = ms.material_unit_id", "stock_final sf" => "sf.material_sub_id = ms.material_sub_id"), "where" => array("md5(mog_id)" => $id, "mog_dt_status" => 1, "md5(sf.project_id)" => $project_id)))->result();
            $this->load->view("warehouse/transaction/material/extend/mog_exit_detail", $data);
        }
    }

    public function get_stock($id = NULL, $project) {
        $data = $this->crud_model->read_fordata(array("table" => "material_sub ms", "join" => array("material_unit mu" => "mu.material_unit_id = ms.material_unit_id", "stock_final sf" => "sf.material_sub_id = ms.material_sub_id"), "where" => array("ms.material_sub_id" => $id, "sf.project_id" => $project)))->row();
        echo json_encode(array('status' => 1, 'data' => $data));
    }

    public function table() {
        $data['sess'] = $this->authentication_root();
        $data['title'] = $this->title;
        $data['header'] = $this->header;

        $child = "(select count(bpm_id) from bpm_stok where bpm_id = e.bpm_id) as child";
        $data['show'] = $this->crud_model->read_fordata(array("table" => "bpm e", "select" => array("*", $child), "join" => array("bpm_ct ec" => "ec.bpm_ct_id = e.bpm_ct_id")))->result();

        $this->load->view("warehouse/master/bpm/bpm_table", $data);
    }

    public function save($id = NULL) {
        $data['sess'] = $this->authentication_root();
        $data['permit'] = $data['sess']['permit'];
        $material_sub = array_filter($this->input->post('material'));
        $mog_dt_volume = array_filter($this->input->post('mog_dt_volume'));
        $mog_dt_note = array_filter($this->input->post('mog_dt_note'));
        $mog_dt_status = $this->input->post('mog_dt_status');
        $code = $this->input->post('code') ? array_filter($this->input->post('code')) : NULL;
//        $mog_dt_convertion = array_filter($this->input->post('mog_dt_convertion'));
//        $mog_dt_qty = array_filter($this->input->post('mog_dt_qty'));
//        $mog_dt_date = array_filter($this->input->post('mog_dt_date'));

        $mog_dt_id = array_filter($this->input->post('mog_dt'));


        $mog_dt_date = NULL;
        $action = $this->input->post('action');

        $mog['transaction_ct_id'] = 2;
        $mog['users_id'] = $this->crud_model->read_fordata(array("table" => "users", "where" => array("md5(users_id)" => $data['sess']['users_id'])))->row()->users_id;
        $mog['project_id'] = $this->input->post("project");
        $mog['actor_id'] = $this->input->post("actor");
        $mog['mog_number'] = $this->input->post("mog_number");
        $mog['mog_number_letter'] = $this->input->post("mog_number_letter");
        $mog['mog_total'] = str_replace('.', '', $this->input->post("mog_total"));

        if (isset($mog)) {
            $actor = $this->crud_model->read_data("actor", array("actor_id" => $mog['actor_id']))->row();
            if (empty($id)) {
                $duplicate = $this->crud_model->read_fordata(array("table" => "mog", "where" => $mog))->num_rows();
                if ($duplicate == 0) {
                    $mog['mog_date'] = date('Y-m-d H:i:s');
                    $mog['mog_status'] = 1;
                    $this->crud_model->insert_data("mog", $mog);
                    $this->recActivity("Membuat BPM No. <b>$mog[mog_number] (<i>$actor->actor_name</i>)</b> on Transaction BPM", "warehouse");
                    $actions = "add";
                    $mogid = $this->crud_model->read_data("mog")->last_row()->mog_id;
                } else {
                    echo json_encode(array('status' => 0, 'msg' => "<div class='alert alert-success alert-dismissable'><button type='button' class='close' data-dismiss='alert' aria-hidden='true'>&times;</button><i class='fa fa-info-circle mg-r-md'></i>Material Usage - BPM failed saved</div>"));
                    exit();
                }
            } else {
                $bpm = $this->crud_model->read_data("mog", array("md5(mog_id)" => "$id"))->last_row();
                $mog['mog_id'] = $bpm->mog_id;
                $this->crud_model->update_data("mog", $mog, "mog_id");
                $this->recActivity("Memperbarui BPM No. <b>$mog[mog_number] (<i>$actor->actor_name</i>)</b> on Transaction BPM", "warehouse");
                $actions = "edit";
                $mogid = $bpm->mog_id;
                if ($data['permit']->access_special == 1) {
                    $diff_volume = array_filter($this->input->post('diff_volume'));
                    $status_volume = array_filter($this->input->post('status_volume'));
                }
            }
        }

        for ($in = 0; $in < count($material_sub); $in++) {
            if (isset($material_sub[$in]) || isset($mog_dt_volume[$in])) {
                $mog_dt[$action[$in]]['material_sub_id'] = $material_sub[$in];
                $mog_dt[$action[$in]]['mog_dt_volume'] = str_replace('.', '', $mog_dt_volume[$in]);
                $mog_dt[$action[$in]]['mog_dt_note'] = !empty($mog_dt_note[$in]) ? $mog_dt_note[$in] : NULL;
                $mog_dt[$action[$in]]['code_id'] = !empty($code[$in]) ? $code[$in] : NULL;
                $mog_dt[$action[$in]]['mog_dt_status'] = isset($mog_dt_status[$in]) ? $mog_dt_status[$in] : NULL;
                $mog_dt[$action[$in]]['mog_dt_date'] = date('Y-m-d H:i:s');
                if (!empty($mog_dt_convertion[$in])) {
                    $mog_dt[$action[$in]]['mog_dt_convertion'] = str_replace('.', '', $mog_dt_convertion[$in]);
                }
                $mog_dt[$action[$in]]['mog_dt_price'] = 0;
                if ($action[$in] == 'edit') {
                    if (isset($mog_dt_id[$in])) {
                        $mog_dt[$action[$in]]['mog_dt_id'] = $mog_dt_id[$in];
                        $this->crud_model->update_data('mog_dt', $mog_dt['edit'], 'mog_dt_id');
                    }
                } else if ($action[$in] == 'add') {
                    if (!empty($id)) {
                        $mog_id = $this->crud_model->read_data("mog", array("md5(mog_id)" => "$id"))->last_row()->mog_id;
                    } else {
                        $mog_id = $this->crud_model->read_data("mog")->last_row()->mog_id;
                    }

                    $mog_dt[$action[$in]]['mog_id'] = $mog_id;

                    $this->crud_model->insert_data('mog_dt', $mog_dt['add']);
                }
//print_r($mog_dt[$action[$in]]);
//echo "<br/>";
                $stock_final = $this->crud_model->read_fordata(array("table" => "stock_final", "where" => array("material_sub_id" => $material_sub[$in], "project_id" => $this->input->post("project"))));

                if ($stock_final->num_rows() == 0) {
                    $stock_fn['insert']['project_id'] = $this->input->post("project");
                    $stock_fn['insert']['material_sub_id'] = $material_sub[$in];
                    $stock_fn['insert']['stock_final_date'] = date('Y-m-d H:i:s');
                    $stock_fn['insert']['stock_final_rest'] = str_replace('.', '', $mog_dt_volume[$in]);
                    $this->crud_model->insert_data("stock_final", $stock_fn['insert']);

                    $stock['stock_rest'] = $stock_fn['insert']['stock_final_rest'];
                } else {
                    $row = $stock_final->row();

                    $stock_fn['update']['stock_final_id'] = $row->stock_final_id;
                    $stock_fn['update']['stock_final_date'] = date('Y-m-d H:i:s');

                    if ($data['permit']->access_special == 1 && $action[$in] == 'edit') {

                        if ($mog_dt[$action[$in]]['mog_dt_status'] == 0) {
                            $del_stock_fn['stock_final_id'] = $stock_fn['update']['stock_final_id'];
                            $del_stock_fn['stock_final_rest'] = $row->stock_final_rest + $mog_dt_volume[$in];
                            $this->crud_model->update_data("stock_final", $del_stock_fn, 'stock_final_id');

                            $del_stock['mog_id'] = $mog['mog_id'];
                            $del_stock['project_id'] = $this->input->post("project");
                            $del_stock['material_sub_id'] = $mog_dt[$action[$in]]['material_sub_id'];
                            $del_stock['stock_date'] = date('Y-m-d H:i:s');
                            $del_stock['stock_entry'] = $mog_dt_volume[$in];
                            $del_stock['stock_rest'] = $del_stock_fn['stock_final_rest'];

                            $this->crud_model->insert_data("stock", $del_stock);
                        }

                        if (isset($status_volume[$in])) {
                            if ($status_volume[$in] == 2) {
                                $stock_fn['update']['stock_final_rest'] = str_replace('.', '', $row->stock_final_rest - $diff_volume[$in]);
//                                $this->crud_model->update_data("stock_final", $stock_fn['update'], 'stock_final_id');
                            } else if ($status_volume[$in] == 3) {
                                $stock_fn['update']['stock_final_rest'] = str_replace('.', '', $diff_volume[$in]) + $row->stock_final_rest;
                            }
                            $this->crud_model->update_data("stock_final", $stock_fn['update'], 'stock_final_id');
                        }
                    } else {
                        $stock_fn['update']['stock_final_rest'] = $row->stock_final_rest - str_replace('.', '', $mog_dt_volume[$in]);
                        $this->crud_model->update_data("stock_final", $stock_fn['update'], 'stock_final_id');
                    }


                    $stock['stock_rest'] = isset($stock_fn['update']['stock_final_rest']) ? $stock_fn['update']['stock_final_rest'] : NULL;
                }

                if (!empty($id)) {
                    $stock['mog_id'] = $mog['mog_id'];
                    $stock['stock_date'] = date('Y-m-d');
                } else {
                    $stock['mog_id'] = $mog_id;
                    $stock['stock_date'] = $mog['mog_date'];
                }
                $stock['project_id'] = $this->input->post("project");
                $stock['material_sub_id'] = $material_sub[$in];
                if ($data['permit']->access_special == 1 && $action[$in] == 'edit') {
                    if (isset($status_volume[$in]) && $status_volume[$in] == 2) {
                        $stock['stock_exit'] = str_replace('.', '', $diff_volume[$in]);
                    } else if (isset($status_volume[$in]) && $status_volume[$in] == 3) {
                        $stock['stock_entry'] = str_replace('.', '', $diff_volume[$in]);
                    }
                } else {
                    $stock['stock_exit'] = str_replace('.', '', $mog_dt_volume[$in]);
                }

                if ($data['permit']->access_special == 1 && $action[$in] == 'edit') {
                    if (isset($status_volume[$in])) {
                        $this->crud_model->insert_data("stock", $stock);
                    }
                } else {
                    $this->crud_model->insert_data("stock", $stock);
                }
            }
        }
//        print_r($_POST);
//exit();
        $this->session->set_flashdata("msgTransMexit", "<div class='alert alert-success alert-dismissable'><button type='button' class='close' data-dismiss='alert' aria-hidden='true'>&times;</button><i class='fa fa-info-circle mg-r-md'></i>  Data BPM - Material Usage successfully $actions</div>");
        echo json_encode(array('status' => 1, 'id' => md5($mogid)));
    }

    public function delete($id = NULL) {
        if (!empty($id)) {
            $get_mog = $this->crud_model->read_fordata(array("table" => "mog", "join" => array("actor a" => "a.actor_id = mog.actor_id"), "where" => array("md5(mog.mog_id)" => $id)))->row();
            $get_dt = $this->crud_model->read_fordata(array("table" => "mog_dt dt", "where" => array("md5(mog_id)" => $id)))->result();

            $data['mog_id'] = $get_mog->mog_id;
            $sts = null;
            foreach ($get_dt as $dt) {
                $stock_final = $this->crud_model->read_fordata(array("table" => "stock_final", "where" => array("project_id" => $get_mog->project_id, "material_sub_id" => $dt->material_sub_id)))->row();

                $stock_fn['stock_final_id'] = $stock_final->stock_final_id;
                if ($get_mog->transaction_ct_id == 1) {
                    $stock_fn['stock_final_rest'] = $stock_final->stock_final_rest - $dt->mog_dt_volume;
                    $sts = "BAPB";
                } else if ($get_mog->transaction_ct_id == 2) {
                    $stock_fn['stock_final_rest'] = $stock_final->stock_final_rest + $dt->mog_dt_volume;
                    $sts = "BPM";
                }
                $this->crud_model->update_data("stock_final", $stock_fn, "stock_final_id");

                $get_stock = $this->crud_model->read_fordata(array("table" => "stock", "where" => array("project_id" => $get_mog->project_id, "material_sub_id" => $dt->material_sub_id)))->last_row();
                $stock['stock_rest'] = $stock_fn['stock_final_rest'];
                $stock['stock_id'] = $get_stock->stock_id;
                $this->crud_model->update_data("stock", $stock, "stock_id");
            }
            $this->recActivity("Menghapus $sts No. <b>$get_mog->mog_number (<i>$get_mog->actor_name</i>)</b> of $sts Information", "warehouse");
            $this->crud_model->delete_data("stock", $data);
            $this->crud_model->delete_data("mog_dt", $data);
            $this->crud_model->delete_data("mog", $data);

            $this->session->set_flashdata("message", "<div class='alert alert-success alert-dismissable'><button type='button' class='close' data-dismiss='alert' aria-hidden='true'>&times;</button><i class='fa fa-info-circle mg-r-md'></i> Equipment successfully removed</div>");
            echo json_encode(array("status" => 1));
        }
    }

}

?>